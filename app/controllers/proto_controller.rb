...
class ProtoController < ApplicationController
  ...
  def analyses_for_property(property_id)
    ...
    analyses.data.each do |a|
      a.results_json = default_results unless a.results_json.present?
    end
    t = render_to_string(partial: 'extract/analyses', formats: [:html], locals: { analyses: analyses })
    sse.write(t)
  end
